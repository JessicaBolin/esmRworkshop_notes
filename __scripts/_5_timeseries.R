...
# Bind everything and 11 yr mean and plot -----------------------------------------
...
plot_ts <- function(ssp, sspletter) {
 ...
 p1 <- ggplot() +
  ...
  geom_rect(data = data.frame(), 
            ...
            alpha = 0.4) +
   geom_rect(data = data.frame(), 
             mapping = aes(xmin = 2040, xmax = 2060, ymin = -Inf, ymax = Inf),
             fill = "grey", 
             alpha = 0.4) +
   geom_rect(data = data.frame(), 
   ...
   annotate("text", x = 2023, y = 13.7,
     ...
     ggsave(p1, 
           filename = paste0(outdir, "/", 
                             ssp, "_SST_timeseries_1993-2100_11yrsmooth.png"),
           width = 8, height = 5)
