...
-   Click a Metagrid UI node (i.e., [LLNL Metagrid](https://aims2.llnl.gov/search))
...
Then, download the script by clicking the **Download** icon next to the `wget` download option. This downloads a shell script (i.e., indicated by the `.sh` file extension) that we will later use to download the file through R interfacing with CDO and the command line.

![](images/downloadwget.png){fig-align="center" width="705"}

Once downloaded, move the file into the `/wget_scripts` folder in your project directory.
...
wget_files <- function(script) {
  setwd(paste0(pth, data_path)) #set directory to where data will be stored
...
```{r}
files <- list.files(paste0(pth, wget_path), #full path where wget scripts are stored
...
Now, we change to multi-session processing, where multiple workers (sessions) are used to download the files concurrently. We then run the function with `future_walk()` , and use the `tictoc` package to time how long this takes. Once all files have downloaded, we change back to single-threaded processing.
...
```{r}
list.files(paste0(pth, data_path))
```

Great! We have successfully downloaded our ESM outputs.

If your scripts downloaded data for 2100-2300, we can remove those files with the `rm` argument in shell:

```{r, eval = F}
system(paste0("rm ", pth, data_path, "/", 
