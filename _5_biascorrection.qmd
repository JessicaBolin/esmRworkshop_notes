...
When bias correcting, we need to calculate the mean baseline climatology across the same time period for both the observed and ESM datasets. Since the **historical** run for the CMIP6 ESMs comprise 1850-2014. Today, we will define our climatology as a 20 year time span. Therefore, we will download twenty years of OISST data, from 1994 to 2014.
...
| 3    | Calculate the OISST climatology for 1993-2014, to be used later for bias correction.                                                                                                                                                                                                                                                                                                                                                 | `_2_OISST_climatology.nc`                                                                     |
| 4    | Calculate the climatology for each historical run of each ESM for 1993-2014. We call this our **ESM baseline**.                                                                                                                                                                                                                                                                                                                      | Two climatology files (one per model) in `__data/bias_correct/esm/_1_climatology`             |
...
input_folder <- paste0(pth, "/__data/oisst_processed")
output_folder <- paste0(pth, "/__data/bias_correct")

# List all processed OISST files
day_ncs <- dir(paste0(input_folder), full.names = TRUE) %>% 
  paste0(., collapse = " ") 
...
cdo_code <- paste0("cdo -s -L ", #Deploy CDO silently and in low memory mode
                   ...
                   " ", 
                   paste0(output_folder, "/_1_OISST_baseline_combined.nc")) # output file 
...
The `.nc` output has 252 layers (one for each month from January 1994 to December 2014).
...
```{r}
rr <- terra::rast(paste0(output_folder, "/_1_OISST_baseline_combined.nc"))
rr
```
...
## Create OISST climatology (1994-2014)

```{r, eval = F}
#| code-fold: true
input_file <- paste0(output_folder, "/_1_OISST_baseline_combined.nc")
output_file <- paste0(output_folder, "/_2_OISST_climatology.nc")
...
![](images/oisstclim.png){fig-align="center" width="400"}

## Create ESM historical climatology (1994-2014)

```{r, eval = F}
#| code-fold: true
infolder = paste0(pth, "/__data/cmip6_processed")
outfolder = paste0(pth, "/__data/bias_correct/esm")
...
filey <- list.files(infolder, pattern = "historical", full.names = T)
...
terra::writeCDF(mean_rr, 
                paste0(outfolder, "/_1_climatology/tos_mo_", model, 
...
This results in an temperature climatology for each ESM, from 1994-2014.

```{r, echo = F}
clim <- terra::rast(paste0(pth, "/__data/bias_correct/esm/_1_climatology/tos_mo_ACCESS-CM2_1994-2014_clim_historical.nc"))
...
anom_esm <- function(model) {
  ...
  # Read in ESM climatology from Step 4
  mean_rr <- rast(paste0(outfolder, "/_1_climatology/tos_mo_", model, 
                    "_1994-2014_clim_historical.nc"))
    
  ### SSPS
  # Then subtract mean climatology from 2015-2100 for both EMSs, creating anomalies
  filey <- list.files(infolder, pattern = "ssp", full.names = T)
  filey <- filey[grep(model, filey)]
  ...
  for (i in 1:length(filey)) {
    ...
    scen <- rast(filey[i])
    anom <- scen - mean_rr 
    writeCDF(anom, paste0(outfolder, "/_2_anomalies/tos_mo_", 
...
The above code results in temperature anomalies for both ESMs (i.e., deviations from the mean). In the image below, the anomalies below represent the ESM projections for `ACCESS-CM2` SSP2-4.5, **minus** the corresponding historical climatology/baseline (1994-2014) created in Step 4.

![](images/esmanomalies.png){fig-align="center" width="400"}
...
```{r, eval = F}
#| code-fold: true
infile <- paste0(pth, "/__data/bias_correct/_2_OISST_climatology.nc")
outfile <- paste0(pth, "/__data/bias_correct/_3_OISST_mask.nc")
...
![](images/oisstmask.png){fig-align="center" width="400"}
...
The resulting four files are ESM anomalies, that been remapped and interpolated using OISST.

```{r, eval = F}
#| code-fold: true
input_folder <- paste0(pth, "/__data/bias_correct/esm/_2_anomalies")
output_folder <- paste0(pth, "/__data/bias_correct/esm/_3_anomalies_remapped")
msk <- paste0(pth, "/__data/bias_correct/_3_OISST_mask.nc")
...
Below left, we see the ESM anomalies created in Step 5, where we subtracted the ESM climatology (1994-2014) from the projections (2015-2100). On the right is the output of this step, representing the remapped anomalies using the OISST mask. Both of these fields represent an `ACCESS-CM2` SSP2-4.5 temperature projection for January 2015.
...
```{r, echo = F}
 ...
 rast(paste0(pth, "/__data/bias_correct/esm/_3_anomalies_remapped/tos_mo_ACCESS-CM2_2015-2100_anom_ssp245_remapped.nc"))[[1]] %>% 
   plot(main = "Remapped anomalies (OISST)")
 maps::map("world", add =T)
```
...
Now, we repeat Steps 5-8 for the 1994-2014 time period from both ESMs. This results in bias-corrected monthly temperature fields for this time period, **from the historical run**.
