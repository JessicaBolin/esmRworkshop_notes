...
Today, we are using ocean temperature output from two ESMs, that are part of the CMIP6 effort: `ACCESS-CM2` [@BI2020] and `IPSL-CM6A-LR` [@BOUCHER2020]. We are using output for two climate scenarios: SSP2-4.5 and SSP5-8.5. These models were selected as representative examples for demonstrating the workflow in this workshop and should not be interpreted as recommended or best-performing models. In your own research, ESM selection is typically guided by the study objectives, region, variable(s) of interest, computational resources, and published model evaluation studies.
