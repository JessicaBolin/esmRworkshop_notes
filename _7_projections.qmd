...
Time to make some projections! We will make projections on the three IPCC AR6 time periods: near-term (2020-2040) and long-term (2080-2100).
...
Now that we have our ensembled projections of SST, we want to inspect the **delta difference**, which is the difference between projected SSTs, and baseline/observed SST experienced between 1994-2014. Essentially, how much will SST change under projected future ocean conditions?

First, we create our historical average, from 1994-2014:

```{r}
#| code-fold: true
#Ensemble average of 1994-2014 for both models
bc_pth <- paste0(pth, "/__data/bias_correct/esm/_4_bias_corrected/")
r1 <- rast(paste0(bc_pth, "tos_mo_ACCESS-CM2_1994-2014_bc_ssp245_remapped.nc"))
r2 <- rast(paste0(bc_pth, "tos_mo_IPSL-CM6A-LR_1994-2014_bc_ssp245_remapped.nc"))
rr <- c(r1, r2)
mean_hist <- mean(rr)
plot(mean_hist, main = "Ensembled SST 1994-2014"); maps::map("world", add = T)
```
...
Woohoo! We've made SST projections for the California region using an ensemble of two ESMs, across two time periods, and, for two climate scenarios.
